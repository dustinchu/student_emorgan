    ...
    List<Widget> items = widget.items
        .map((value) => _MenuItem(
              ...
              child: Container(
                ...
                color:
                    clickitem == value ? Color(0xFFC8C1EF) : Colors.transparent,
                height: 40,
                alignment: Alignment.center,
                child: Text("$value"),
              ),
              click: (value) {
                setState(() => clickitem = value);
...
class __MenuState<T> extends State<_Menu<T>> {
  ...
  @override
  Widget build(BuildContext context) {
    var bookStatus = Provider.of<BookStatus>(context);
...
class _MenuItem<T> extends StatelessWidget {
  final T value;
  final Widget child;
  final MenuItemBuilder click;

  const _MenuItem(
      {this.value, @required final this.child, @required this.click});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: new BoxDecoration(
        color: Colors.transparent,
        border: new Border.all(color: Color(0xFFC8C1EF), width: 2),
      ),
      child: InkWell(
          child: child,
          onTap: () {
